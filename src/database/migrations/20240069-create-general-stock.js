...
module.exports = {
  async up(queryInterface, Sequelize) {
    // General Stock Items
    await queryInterface.createTable('general_stock_items', {
    ...
    // General Stock Entries (IN)
    await queryInterface.createTable('general_stock_entries', {
      id: { type: Sequelize.UUID, defaultValue: Sequelize.UUIDV4, primaryKey: true, allowNull: false },
      stockItemId: { type: Sequelize.UUID, allowNull: false, references: { model: 'general_stock_items', key: 'id' }, onUpdate: 'CASCADE', onDelete: 'RESTRICT' },
    ...
    // General Stock Sorties (OUT)
    await queryInterface.createTable('general_stock_sorties', {
      id: { type: Sequelize.UUID, defaultValue: Sequelize.UUIDV4, primaryKey: true, allowNull: false },
      stockItemId: { type: Sequelize.UUID, allowNull: false, references: { model: 'general_stock_items', key: 'id' }, onUpdate: 'CASCADE', onDelete: 'RESTRICT' },
    ...
    await queryInterface.addIndex('general_stock_items', ['isActive']);
    await queryInterface.addIndex('general_stock_entries', ['stockItemId']);
    await queryInterface.addIndex('general_stock_sorties', ['stockItemId']);
    await queryInterface.addIndex('general_stock_sorties', ['status']);
  },
